...
- 状态：P0 已实现，随本地内测验收维护
- 更新日期：2026-08-09
- 相关文档：[当前状态与下一步](./STATUS.md) · [精简 PRD](./PRD.md) · [状态与交互图](./INTERACTIONS.md)
...
语言规则由 Blink 根据 `original_prompt` 逐请求生成，放在模式规则之后，且不能被内置模式或自定义偏好覆盖：

- 输入包含中文字符：`optimized_prompt` 必须以中文为主要语言；中英混合内容也使用中文，但 API、RLHF、US、ChatGPT 等英文缩写、产品名和专有名词可以原样保留。
- 输入不包含中文字符：`optimized_prompt` 保持输入的主要自然语言；英文输入必须输出英文，不得翻译成中文。缩写、产品名和专有名词不影响语言判定。
...
7. 输入与输出的中文字符存在性必须一致：纯英文等非中文输入不得产生中文输出；包含中文的输入（包括中英混合）不得产生纯英文输出。
8. 解析或校验失败返回 `INVALID_RESPONSE`，不进行自动重试。

语言校验仅阻止中文与非中文之间的明确错配，不尝试识别所有自然语言，也不自动校验专有名词或所有数字；后者由固定 Prompt 语料验收承担。
